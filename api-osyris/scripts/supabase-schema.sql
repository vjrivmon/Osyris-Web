-- 🏕️ OSYRIS SCOUT MANAGEMENT - ESQUEMA SUPABASE
-- Script de creación de tablas para PostgreSQL/Supabase
-- Fecha: 2025-09-29

-- ========================================
-- LIMPIEZA (opcional - solo si necesitas recrear)
-- ========================================
...
CREATE TABLE IF NOT EXISTS paginas (
  ...
  imagenes_galeria TEXT, -- JSON array de URLs
...
-- Política de lectura pública para secciones
CREATE POLICY "Secciones son públicas" ON secciones
  FOR SELECT USING (true);

-- Política de lectura pública para páginas visibles
CREATE POLICY "Páginas visibles son públicas" ON paginas
...
-- Más políticas se pueden agregar según necesidades
...
BEGIN
  ...
  SELECT
    ...
    (SELECT COUNT(*) FROM secciones)::BIGINT as total_secciones,
    (SELECT COUNT(*) FROM actividades)::BIGINT as total_actividades,
    (SELECT COUNT(*) FROM documentos)::BIGINT as total_documentos,
    (SELECT COUNT(*) FROM paginas)::BIGINT as total_paginas;
END;
$$ LANGUAGE plpgsql;
